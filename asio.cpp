...
auto AsyncReadSome(net::ip::tcp::socket &Socket, MutableBuffer &&Buffer) {
  struct FAwaiter {
    FAwaiter(net::ip::tcp::socket &Socket, MutableBuffer &&Buffer) : Socket(Socket), Buffer(std::forward<MutableBuffer>(Buffer)) { }
    ...
    bool await_suspend(stdx::coroutine_handle<> Handle) {
      Socket.async_read_some(Buffer, [this, Handle](auto &&Code, auto &&Received) {
...
auto AsyncWrite(net::ip::tcp::socket &Socket, ConstBuffer &&Buffer) {
  struct FAwaiter {
    FAwaiter(net::ip::tcp::socket &Socket, ConstBuffer &&Buffer) : Socket(Socket), Buffer(std::forward<ConstBuffer>(Buffer)) { }
    ...
    bool await_suspend(stdx::coroutine_handle<> Handle) {
      net::async_write(Socket, Buffer, [this, Handle](auto &&Code, auto &&Transferred) {
...
auto AsyncAccept(net::ip::tcp::acceptor &Acceptor) {
  struct FAwaiter {
    explicit FAwaiter(net::ip::tcp::acceptor &Acceptor) : Acceptor(Acceptor), Socket(Acceptor.get_executor().context()) { }
    ...
    bool await_suspend(stdx::coroutine_handle<> Handle) {
      Acceptor.async_accept(Socket, [this, Handle](auto &&Code) {
        this->Code = std::forward<decltype(Code)>(Code);
        Handle.resume();
      });
      return true;
...
public:
  ...
  void operator()(net::io_context &Context) && {
    net::post(Context, [Handle = std::exchange(Handle, nullptr)]() {
      Handle.resume();
    });
  }
private:
...
int main() {
  ...
  Signals.async_wait([&Context](auto &&, auto &&) {
    Context.stop();
  });
